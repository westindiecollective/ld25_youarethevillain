...
[CustomEditor (typeof(Lava3))]

public class Lava3Editor : Editor 
{	
	...
	public SerializedProperty m_UVOffsetA;
	public SerializedProperty m_UVOffsetB;

	public SerializedProperty m_FoamWaveParams;
	public SerializedProperty m_UnderwaterCheckOffset;
	
	public SerializedProperty m_Water3Material;
	...
	{		
		...
		m_Water3Material = m_SerObj.FindProperty("m_Water3Material");
		...
		m_UVOffsetA = m_SerObj.FindProperty("m_UVOffsetA");
		m_UVOffsetB = m_SerObj.FindProperty("m_UVOffsetB");

		m_FoamWaveParams = m_SerObj.FindProperty("m_FoamWaveParams");
		
		m_Displacement = LavaManager.Instance().GetMaterialTexture("_DisplacementHeightMap");
		m_2ndDisplacement = LavaManager.Instance().GetMaterialTexture("_SecondDisplacementHeightMap");
		
		m_NormalMap = LavaManager.Instance().GetMaterialTexture("_BumpMap");
		m_FoamMap = LavaManager.Instance().GetMaterialTexture("_ShoreTex");
		m_CubeFallback = LavaManager.Instance().GetMaterialTexture("_ReflectiveColorCube");
		m_Fallback = LavaManager.Instance().GetMaterialTexture("_MainTex");
	} 
    ...
    public override void OnInspectorGUI() 
	{
		...
		EditorGUILayout.PropertyField (m_Water3Material, new GUIContent("Water Material"));
		...
		{
			EditorGUILayout.BeginHorizontal();
			m_UVOffsetA.vector2Value = EditorGUILayout.Vector2Field("Texture 1 X and Y Velocity", m_UVOffsetA.vector2Value);
			m_UVOffsetB.vector2Value = EditorGUILayout.Vector2Field("Texture 2 X and Y Velocity", m_UVOffsetB.vector2Value);
			EditorGUILayout.EndHorizontal();
			float foamTempY;
			float foamTempX;
			float foamTempZ;
		...
		LavaManager.Instance().SetMaterialColor(
			"_RefrColorDepth", 
			EditorGUILayout.ColorField("Depth Fade", LavaManager.Instance().GetMaterialColor("_RefrColorDepth")) );
		LavaManager.Instance().SetMaterialColor(
			"_SpecularColor", 
			EditorGUILayout.ColorField("Specular", LavaManager.Instance().GetMaterialColor("_SpecularColor")) );
		LavaManager.Instance().SetMaterialColor(
			"_RefrColor", 
			EditorGUILayout.ColorField("Fallback", LavaManager.Instance().GetMaterialColor("_RefrColor")) );
			//_UnderwaterColor
		LavaManager.Instance().SetMaterialColor(
			"_UnderwaterColor", 
			EditorGUILayout.ColorField("Underwater", LavaManager.Instance().GetMaterialColor("_UnderwaterColor")) );
		...
		if(m_WaterDisplacement.boolValue) {
			...
			EditorGUILayout.BeginHorizontal();
			LavaManager.Instance().m_DisplacementTiling = EditorGUILayout.FloatField(new GUIContent("Tiling"),LavaManager.Instance().m_DisplacementTiling);
			LavaManager.Instance().m_SmallWavesSpeed = EditorGUILayout.FloatField(new GUIContent("Speed"),LavaManager.Instance().m_SmallWavesSpeed);	
			EditorGUILayout.EndHorizontal();
			EditorGUILayout.BeginHorizontal();
			LavaManager.Instance().m_HeightDisplacement = EditorGUILayout.FloatField(new GUIContent("Height"),LavaManager.Instance().m_HeightDisplacement);
			LavaManager.Instance().m_NormalsDisplacement = EditorGUILayout.FloatField(new GUIContent("Normals Strength"),LavaManager.Instance().m_NormalsDisplacement);			
			EditorGUILayout.EndHorizontal();
			...
			*/
			...
			LavaManager.Instance().SetMaterialVector("_DisplacementXz", displacementXz);
			LavaManager.Instance().SetMaterialVector("_Displacement", displacement4);
