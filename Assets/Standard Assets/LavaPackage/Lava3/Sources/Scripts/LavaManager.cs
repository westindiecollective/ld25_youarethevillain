...
public class LavaManager : MonoBehaviour {
	...
	// singleton
	private static LavaManager s_Instance = null;
	...
	public static LavaManager Instance() {
		if(s_Instance==null) {
			s_Instance = FindObjectOfType(typeof (LavaManager)) as LavaManager;
		} 
		if(s_Instance==null) {
			GameObject obj = new GameObject("LavaManager");
			s_Instance = obj.AddComponent(typeof (LavaManager)) as LavaManager;
			Material m = (FindObjectOfType(typeof (Lava3)) as Lava3).m_Water3Material;
			((LavaManager)obj.GetComponent(typeof (LavaManager))).m_SharedWaterMaterial = m;
			Debug.Log ("could not locate an LavaManager object. generated.");		
		}
	...
	public float GetDisplaceMeshAmountAt(Vector3 pos,Transform t) {
		if(m_CpuDisplacementModel == LavaManager.CpuDisplacementModel.None)
			return 0.0F;
		else
